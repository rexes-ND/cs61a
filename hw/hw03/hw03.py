...
def interleaved_sum(n, odd_term, even_term):
    ...
    def helper(start, end, f1, f2):
        if start == end:
            return f1(start)
        return f1(start) + helper(start + 1, end, f2, f1)

    return helper(1, n, odd_term, even_term)
...
def count_coins(total):
    ...
    def helper(total, coin):
        ...
        if total == 0:
            return 1
        if total >= coin:
            return helper(total, next_smaller_coin(coin)) + helper(total - coin, coin)
        else:
            return helper(total, next_smaller_coin(coin))
