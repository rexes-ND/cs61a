...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    new_branches = []
    for branch in branches(t):
        new_branches.append(replace_loki_at_leaf(branch, lokis_replacement))
...
def distance(city_a, city_b):
    ...
    "*** YOUR CODE HERE ***"
    return sqrt(
        pow(get_lat(city_a) - get_lat(city_b), 2)
        + pow(get_lon(city_a) - get_lon(city_b), 2)
    )
