...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        fname = signature.first
        formals = signature.rest
        while signature.rest is not nil:
            if not scheme_symbolp(signature.first):
                raise SchemeError("non-symbol: {0}".format(signature.first))
            signature = signature.rest
        body = expressions.rest
        lmbda = LambdaProcedure(formals, body, env)
        env.define(fname, lmbda)
        return fname
        # END PROBLEM 10
    else:
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            if clause.rest is nil:
                return test
            if isinstance(clause.rest.first, Pair) and len(clause) > 2:
                return eval_all(clause.rest, env)
            return scheme_eval(clause.rest.first, env, tail=True)
            # END PROBLEM 13
        expressions = expressions.rest
