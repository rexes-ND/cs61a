...
def accuracy(typed, source):
    ...
    for i in range(min(len(typed_words), len(source_words))):
        if typed_words[i] == source_words[i]:
            matched += 1
    return matched / len(typed_words) * 100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    least_diff = diff_function(typed_word, word_list[0], limit)
    least_diff_word = word_list[0]
    for word in word_list:
        word_diff = diff_function(typed_word, word, limit)
        if word_diff < least_diff:
            least_diff = word_diff
...
def time_per_word(words, timestamps_per_player):
    ...
    for timestamps in timestamps_per_player:
        ...
        for i in range(1, len(timestamps)):
            times_per_player.append(timestamps[i] - timestamps[i - 1])
        times.append(times_per_player)
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        ...
        fastest_player_time = time(match, fastest_player_index, word_index)
        for player_index in player_indices:
            player_time = time(match, player_index, word_index)
            if player_time < fastest_player_time:
                fastest_player_time = player_time
                fastest_player_index = player_index
        res[fastest_player_index].append(word)
    return res
    # END PROBLEM 10
